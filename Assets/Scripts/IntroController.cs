...
public class IntroController : MonoBehaviour
{
    ...
    public void PlayIntro(int startingPlayer, System.Action onComplete)
    {
        ...
        StartCoroutine(WatchdogTimer(onComplete));
    }

    private IEnumerator WatchdogTimer(System.Action onComplete)
    {
        // 5 saniyelik Watchdog / Güvenlik Köpeği
        // Eğer 5 saniye içinde IntroSequence onComplete çağırmazsa, kilitlenmiş demektir.
        yield return new WaitForSeconds(5f);

        if (!isIntroComplete)
        {
            Debug.LogWarning("[Watchdog] Intro animasyonu zaman aşımına uğradı veya kilitlendi! Zorla başlatılıyor...");
    ...
    private IEnumerator IntroSequence(int startingPlayer, System.Action onComplete)
    {
        try {
            ...
            Debug.LogError("BuildIntroCanvas failed: " + e.Message);
        }
    ...
    private void CreateFragments()
    {
        ...
        {
            Debug.LogError("[IntroController] Parçacıklar oluşturulurken hata: " + e.Message);
        }
    }
